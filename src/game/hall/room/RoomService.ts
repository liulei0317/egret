module game {
	export class RoomService {
		private static intstance: RoomService = null;
		private constructor() {

		}

		public static getInstance() {
			if (RoomService.intstance == null) {
				RoomService.intstance = new RoomService();
			}
			return RoomService.intstance;
		}

		public createRoom(roomInfo) {
			var obj: any = {};
			obj.msgRoom = roomInfo;
			var requestDomain = DomainUtils.getRequestDomain(MsgConstant.HALL, MsgConstant.CMD_CREATE_ROOM, obj);
		...
		public joinRoomCallBack(msgDomain: MsgDomain) {
			if (msgDomain.code == CmdResultType.SUCCESS) {
				...
				} else if (errorCode == MsgConstant.ERROR_bless_score_limit) {
					DialogManager.getInstance().popUp1(Strings.club_error_bless_score_limit);
				}
				else if (errorCode == MsgConstant.ERROR_club_black_list) {
					DialogManager.getInstance().popUp1(Strings.club_forbid_game);
				}
			}
